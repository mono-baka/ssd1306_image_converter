...
public class sketch_170926a_pde extends PApplet {
...
public void fileSelected(File selection) {
  if (selection == null) {
    ...
    for(int i=0;i<dimension;i++){
      int bright = PApplet.parseInt(brightness(img.pixels[i]));
      hist[bright]++;
    }
    textAlign(LEFT, TOP);
    textSize(24);
...
public void draw() {
  if(img!=null){
    ...
    for (int i = 0; i < 256; i++) {
      ...
      int y = PApplet.parseInt(map(hist[i], 0, histMax, img.height-hist_margin*2, 0));
...
public void mouseWheel(MouseEvent e){
  ...
  if((right_line-left_line)/(float)(under_line-up_line)<128/64.0f){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_170926a_pde" };
    if (passedArgs != null) {
